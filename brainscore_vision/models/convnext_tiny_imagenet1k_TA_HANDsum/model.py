...
def get_model(name):
    assert name == 'convnext_tiny_imagenet1k_TA_HANDsum'
    fileName='ConvNeXt-tiny_ImageNet_None_TrivialAug+RandErase0.1+mixup_None_HAND_Conv_LayerNorm_StridedConv_CELoss_None'
    ...
    #model = import_weights('brainscore_vision/models/convnext_tiny_imagenet1k_TA_HANDsum/'+fileName+'.pt', model, device)
    ...
    wrapper = PytorchWrapper(identifier='convnext_tiny_imagenet1k_TA_HANDsum', model=model, preprocessing=preprocessing)
...
def get_layers(name):
    ...
    return ['downsample_layers.0', 'downsample_layers.1', 'downsample_layers.2', 'downsample_layers.3', 'stages.0.0', 'stages.0.1', 'stages.0.2', 'stages.1.0', 'stages.1.1', 'stages.1.2', 'stages.2.0', 'stages.2.1', 'stages.2.2', 'stages.2.3', 'stages.2.4', 'stages.2.5', 'stages.2.6', 'stages.2.7', 'stages.2.8', 'stages.3.0', 'stages.3.1', 'stages.3.2', 'head'] #ConvNeXt
...
class HAND(nn.Module):
    # A custom activation function that applies
    ...
    def __init__(self, numChannels):
        ...
        self.logpoolWeight = nn.Parameter(torch.log(torch.ones(1)/numChannels))

    def forward(self, x):
        # leant params are constrained to be non-negative
        exponent = 2 #torch.exp(self.logexponent) + 1
        semiSaturation = torch.exp(self.logsemiSaturation)
        poolWeight = torch.exp(self.logpoolWeight)
        ...
        #                         i.e. across columns)
        poolActivation = torch.sum(x, dim=1, keepdim=True)
        poolActivation = poolWeight * poolActivation

        x /= (semiSaturation + poolActivation)
        return x
