...
def get_model_list():
    return ['resnet50_expCBLS_wzc']

def get_model(name):
    assert name == 'resnet50_expCBLS_wzc'
    ...
    file_path = load_file(bucket="brainscore-storage", folder_name="brainscore-vision/models/user_755/",
                          relative_path="expCBLScheckpoint_89.pth",
                          version_id="tvWyZyRTIxlyvP9K1diAVDc69_su8uCn",
                          ),
    ...
    wrapper = PytorchWrapper(identifier='resnet50_expCBLS_wzc', model=model, preprocessing=preprocessing)
