...
def get_model_list():
    return ['resnet50_hvs_wzc']

def get_model(name):
    assert name == 'resnet50_hvs_wzc'

    file_path = load_file(bucket="brainscore-storage", folder_name="brainscore-vision/models/user_672/",
                          relative_path="resnet50_hvs_wzc_checkpoint_89.pth",
                          version_id="Oe49fSnIOTmgxbuz99jB4h8lrWInljbC",
                          ),
    ...
    wrapper = PytorchWrapper(identifier='resnet50_hvs_wzc', model=model, preprocessing=preprocessing)
