...
def get_model_list():
    return ['resnet50_ewcon_wzc']

def get_model(name):
    assert name == 'resnet50_ewcon_wzc'

    file_path = load_file(bucket="brainscore-storage", folder_name="brainscore-vision/models/user_672/",
                          relative_path="resnet50_ewcon_wzc_checkpoint_89.pth",
                          version_id="jYZZuo_z9nExp.aPSGwNQ2WTFrImNFFp",
                          ),
    ...
    wrapper = PytorchWrapper(identifier='resnet50_ewcon_wzc', model=model, preprocessing=preprocessing)
