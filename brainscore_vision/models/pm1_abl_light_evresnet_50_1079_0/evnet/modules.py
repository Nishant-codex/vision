...
class LightAdaptation(nn.Module):
    def __init__(self, in_channels, p_channels, m_channels, kernel_sizes, radii, light_adapt_mode, n, pool_dark, gaussian=True):
        super().__init__()
        num_conv = bool(p_channels) + bool(m_channels)
        self.in_channels = in_channels
        ...
        self.epsilon = 0.05 if pool_dark else EPSILON
        if n is not None:
        ...
        print('[RetinaBlock] Using LOCAL Light Adaptation')
    ...
    def forward(self, x):
        if self.light_adapt_mode=='carandini':
            ...
            num = x.unsqueeze(1)
            denom = self.mean(x).unsqueeze(2) + num + self.epsilon
            return  (num / denom).reshape(x.shape[0], -1, *x.shape[2:]) - .5
        elif self.light_adapt_mode=='naka':
            num = x.unsqueeze(1) ** self.n.unsqueeze(2)
            denom = (self.mean(x) ** self.n).unsqueeze(2) + num + self.epsilon
            return  (num / denom).reshape(x.shape[0], -1, *x.shape[2:]) - .5
        else:
            raise ValueError(f'{self.light_adapt_mode} is not a valid light adaptation mode.')
...
class RetinaBlock(nn.Module):
    def __init__(
        ...
        k_exc, fano_factor=.4, noise_mode=None, light_adapt_mode='carandini', exp_la=None, pool_dark=True,
        ...
        if with_light_adapt:
            self.light_adapt = LightAdaptation(in_channels, p_channels, m_channels, kernel_la, radius_la, light_adapt_mode, exp_la, pool_dark)
        if with_dog:
...
class EVBlock(nn.Module):
    def __init__(self, retinablock, voneblock, backend_in_features, lgn_to_v2=False):
        super().__init__()
        self.retinablock = retinablock
        ...
        bottleneck_out_features = backend_in_features if not self.lgn_to_v2 else backend_in_features - retinablock.p_channels - retinablock.m_channels
        self.bottleneck = nn.Conv2d(bottleneck_in_features, bottleneck_out_features, 1, bias=False, groups=1)
    
    def forward(self, x):
        if self.lgn_to_v2:
            y_lgn = self.retinablock(x)
