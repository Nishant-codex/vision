...
def get_model_list():
    return ['resnet50_st_no_wzc_seed5']

def get_model(name):
    assert name == 'resnet50_st_no_wzc_seed5'
    ...
    file_path = load_file(bucket="brainscore-storage", folder_name="brainscore-vision/models/user_755/",
                      relative_path="st_no_seed5_checkpoint_89.pth",
                      version_id="ID1T7sGQ0lFNAHis2BYsjIalRdJVBgqG",
                      ),
    ...
    wrapper = PytorchWrapper(identifier='resnet50_st_no_wzc_seed5', model=model, preprocessing=preprocessing)
