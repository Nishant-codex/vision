...
def get_model_list():
    return ['resnet50_st_wzc_noseed_2_323']

def get_model(name):
    assert name == 'resnet50_st_wzc_noseed_2_323'

    file_path = load_file(bucket="brainscore-storage", folder_name="brainscore-vision/models/user_672/",
                          relative_path="resnet50_noseed2_checkpoint_89.pth",
                          version_id="gdKl7klLSzJ0d2G7b9SK4i9lEpwOhAfq",
                          ),
    ...
    wrapper = PytorchWrapper(identifier='resnet50_st_wzc_noseed_2_323', model=model, preprocessing=preprocessing)
