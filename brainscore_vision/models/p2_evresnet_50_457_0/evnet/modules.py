...
class DoG(nn.Module):
    def __init__(self, in_channels, p_channels, m_channels, kernel_size, r_c,  r_s, opponency, with_light_adapt):
        ...
        if bool(m_channels) and not with_light_adapt:
        ...
        for i in range(p_channels+m_channels):
            ...
            conv.weight.data[filter_i] = torch.stack(kernels, dim=0) # (1,51,51) = (3,51,51)
...
class LightAdaptation(nn.Module):
    def __init__(self, in_channels, p_channels, m_channels, kernel_sizes, radii, light_adapt_mode, gaussian=True):
        super().__init__()
        num_conv = bool(p_channels) + bool(m_channels)
        self.in_channels = in_channels
        self.kernel_sizes = kernel_sizes
        self.radii = radii
        self.light_adapt_mode = light_adapt_mode
    ...
    def forward(self, x):
        if self.light_adapt_mode=='carandini':
            num = x.unsqueeze(1) - self.mean(x).unsqueeze(2)
            denom = self.mean(x).unsqueeze(2) + EPSILON
            return  (num / denom).reshape(x.shape[0], -1, *x.shape[2:])
        elif self.light_adapt_mode=='weber':
            num = x.unsqueeze(1)
            denom = self.mean(x).unsqueeze(2) + num + EPSILON
            return  (num / denom).reshape(x.shape[0], -1, *x.shape[2:]) - .5
        if self.light_adapt_mode=='naka':
...
class RetinaBlock(nn.Module):
    def __init__(
        ...
        kernel_la, radius_la, kernel_cn, radius_cn, c50_cn, exp_cn,
        k_exc, fano_factor=.4, noise_mode=None, light_adapt_mode='carandini',
        ...
        if with_light_adapt:
            self.light_adapt = LightAdaptation(in_channels, p_channels, m_channels, kernel_la, radius_la, light_adapt_mode)
        if with_dog:
...
class AdvTrainedModel(nn.Module):
    """
    Wrapper class for adversarially trained model using ART.
    """
    def __init__(
        self, model, num_classes, input_size,
        preprocessing, optimizer, k, **attack_kwargs
    ):
        super().__init__()
        self.model = model
        ...
        self.attack = ProjectedGradientDescent(
            estimator=self.ensemblize(self.classifier, k=k),
            verbose=False, **attack_kwargs
            )
        self.val_attacker = True
        self.preprocessing = T.Normalize(*preprocessing)
    
    def ensemblize(self, classifier, k):
        return self.classifier if k==1 else EnsembleClassifier(
            classifiers=[self.classifier for _ in range(k)],
            channels_first=True,
            clip_values=(0, 1)
            )
    
    def toggle_val_attacker(self):
        self.val_attacker = not self.val_attacker

    def forward(self, x, y=None):
        prev_training = self.training 
        if not prev_training and not self.val_attacker:
            return self.model(x)
        ...
        if prev_training:
            self.model.train()
        x_adv = torch.from_numpy(x_adv).to(device=torch.cuda.current_device(), dtype=torch.float32)
        return self.model(self.preprocessing(x_adv))
