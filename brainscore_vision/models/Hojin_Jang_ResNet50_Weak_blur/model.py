...
def get_model_list():
    return ['Hojin_Jang_ResNet50_Weak_blur']

def get_model(name):
    assert name == 'Hojin_Jang_ResNet50_Weak_blur'
    ...
    file_path = load_file(bucket="brainscore-storage", folder_name="brainscore-vision/models/user_672/",
                      relative_path="ResNet50_Weak-blur.pth",
                      version_id="Y6_MsLhOTMSRpT_nAVQm6wXrNYtiu0pU",
                      ),
    ...
    wrapper = PytorchWrapper(identifier='Hojin_Jang_ResNet50_Weak_blur', model=model, preprocessing=preprocessing)
